...
def BFS_u_to_v_complete2(G, u, V_t, s):
  forest = {u: None}
  found, t_e = BFS_u_to_v2(G, forest, u, V_t, s)
  return forest, found, t_e
...
def BFS_u_to_v2(g, discovered, source, V_t, s):
  level = [source]
  while len(level) > 0:
    next_level = []
    for u in level:
...
def buildFlowNet2(V: dict, E:dict):
    G = graph.Graph(directed=True)
    ...
    for v in V:
        vertex[v] = G.insert_vertex_directed(v)
        if(count % 2 == 0):
            G.insert_edge_fast(vertex["s"], vertex[v], float("inf"))
        else:
            G.insert_edge_fast(vertex[v], vertex[-1],  float("inf"))
        count += 1
...
def Augment2(G: graph.Graph, path: list, forest: dict):
    bottleneck = forest[path[1]].element()
    for v in path:
        if forest[v] is None:
            pass
...
def maxFlow2(G: graph.Graph, V: dict):
    t_dict = G.get_incoming(V[-1])
    s_dict = G.get_outgoing(V["s"])
    ...
    for s_e in s_dict_copy:
        ...
        while True:
            ...
            if s_e not in s_dict:
                break
            forest, found, t_e = BFS_u_to_v_complete2(G, s_e, t_dict, s_dict)
            if not found:
                break
            forest[source] = None
            forest[s_e] = e_s_dict
...
def facebook_friend2(V: dict, E:dict):
    G, vertex = buildFlowNet2(V,E)
    return maxFlow2(G, vertex)